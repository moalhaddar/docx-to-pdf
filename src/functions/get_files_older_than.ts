	...
	new Promise(
		(res, rej) => 
			stat(file_path, (err, meta) =>{
				...
				const fileCreationTime = ms_to_sec(meta.ctime.getTime() / 1000);
				return res(currentTime - fileCreationTime);
			})	
	)

export const get_files_older_than_n_seconds: 
	(file_paths: string[], target_age_in_seconds: number, logger: logger_function_type, fk: () => void, sk: (file_paths_older_than: string[]) => void) => void
